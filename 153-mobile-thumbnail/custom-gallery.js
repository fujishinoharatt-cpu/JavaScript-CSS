(function () {
    ...
    kintone.events.on(events, function (event) {
        ...
        // 1. 現象と工程は「152-マスタ(不適合工程)」アプリ(アプリID:152)からAPIで全件取得する
        var xhrApp152 = new XMLHttpRequest();
        // 500件以内想定で一旦 limit 500
        xhrApp152.open('GET', '/k/v1/records.json?app=152&query=' + encodeURIComponent('limit 500'), true);
        xhrApp152.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
        // Kintone API Token for App 152 (must be generated and supplied by user. Using a placeholder or requesting the user to generate it)
        // Note: For now, if no token is needed for the user themselves, omitting the token header if session auth works, 
        // BUT normally API tokens are cleaner. We will assume session auth works for the frontend JS within the same domain unless informed otherwise.
        // Actually, previous App 92 used a token 'grt3LJb0VeSMDIUmvoraZVswynPHx1q67uBmnZoR'. Let's ask the user if they have an API token for 152, or try session auth (which is default for XMLHttpReq in kintone frontend).
        // Best approach for kintone frontend is kintone.api() which inherits session:
        kintone.api(kintone.api.url('/k/v1/records', true), 'GET', { app: 152, query: 'limit 500' }).then(function (resp) {
            var phenomSet = {};
            var processSet = {};

            resp.records.forEach(function (r) {
                var pName = r['現象名'] && r['現象名'].value;
                var procName = r['工程名'] && r['工程名'].value;

                if (pName) phenomSet[pName] = true;
                if (procName) processSet[procName] = true;
            });

            var phenomValues = Object.keys(phenomSet).sort();
            var processValues = Object.keys(processSet).sort();

            selPhenom.innerHTML = '<option value="">すべての現象</option>';
            phenomValues.forEach(function (o) {
            ...
            processValues.forEach(function (o) {
                var opt = document.createElement('option');
                opt.value = o; opt.innerText = o;
            ...
            console.error("152マスタ取得エラー", err);
            selPhenom.innerHTML = '<option value="">すべての現象 (読込エラー)</option>';
            selProcess.innerHTML = '<option value="">すべての工程 (読込エラー)</option>';
        });
